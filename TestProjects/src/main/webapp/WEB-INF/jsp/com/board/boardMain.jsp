...
$(function(){
	$("#mainGrid").jqGrid({
		...
		colModel:[
					{name:'bIdx', index:'bIdx', align:"center", width:"50px"},
...
function formatOpt(cellvalue, options, rowObject) {
	...
	str += "<button type='button' class='btn btn-default btn-sm' style='padding:2px 10px 2px; border:1px solid gray; z-index:1;' onclick=\"javascript:fn_update_allow('" + rowObject.uIdx + "')\">수정</button>";
	str += "<button type='button' class='btn btn-default btn-sm' style='padding:2px 10px 2px; border:1px solid gray; z-index:1;' onclick=\"javascript:fn_board_delete('" + rowObject.bIdx + "','" + rowObject.uIdx + "')\">삭제</button>";
	str += "</div>"
	return str;
}
...
/* 게시글 삭제 */
function fn_board_delete(bIdx, uIdx) {
	if('${sessionVo.uIdx}' != uIdx) {
		...
		<div class="widget-body" style="padding:30px;">
			<fieldset>
				<div class="form-group" style="flex:center;">
					<label><span class="widget-icon"><i class="fa fa-list-ul"></i>&nbsp;&nbsp;&nbsp;CONTENTS</span></label>
				</div>
			</fieldset>
...
<form class="form-horizontal" id="frm_update_board" name="frm_update_board" onsubmit="return false">
	<div class="board_info" id="content" style="width:1000px; height:500px; margin:auto;">
		<div class="widget-body" style="padding:30px;">
			...
			<fieldset>	
				<legend style="padding-top:0px; font-size:14px; margin-bottom:5px; margin-top:15px;">WRITER</legend>
				<div style="width:100px; text-align: left;">
					<input class="form-control input-sm" id="view_uNm" readonly="readonly"/>					
				</div>	
